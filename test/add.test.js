  ...
  describe('positive tests', () => {
    ...
    describe('edge cases', () => {
      it('should handle very large and very small numbers', () => {
        expect(add(Number.MAX_SAFE_INTEGER, 1)).to.equal(Number.MAX_SAFE_INTEGER + 1, 'MAX_SAFE_INTEGER + 1 should equal MAX_SAFE_INTEGER + 1');
        expect(add(Number.MIN_SAFE_INTEGER, -1)).to.equal(Number.MIN_SAFE_INTEGER - 1, 'MIN_SAFE_INTEGER - 1 should equal MIN_SAFE_INTEGER - 1');
      });

      it('should handle operations with Infinity', () => {
        expect(add(Infinity, 1)).to.equal(Infinity, 'Infinity + 1 should equal Infinity');
        expect(add(1, Infinity)).to.equal(Infinity, '1 + Infinity should equal Infinity');
        expect(add(-Infinity, 1)).to.equal(-Infinity, '-Infinity + 1 should equal -Infinity');
        expect(add(1, -Infinity)).to.equal(-Infinity, '1 + -Infinity should equal -Infinity');
      });
    });
